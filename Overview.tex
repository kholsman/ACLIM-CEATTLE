...
\subsection{1.1 ACLM Overview}\label{aclm-overview}
...
\begin{Highlighting}[]
    ...
    \KeywordTok{head}\NormalTok{(sim_msm)}
    
    \CommentTok{# select subset of data for analyses:}
  \CommentTok{# sub <- sim_msm%>%}
  \CommentTok{#   filter(recMode==rset,hMode%in%rfset, Scenario==scn)}

    
    \NormalTok{dat_2_5_3  <-}\StringTok{ }\NormalTok{sim_msm%>%}\KeywordTok{filter}\NormalTok{(recMode==}\KeywordTok{as.character}\NormalTok{(rset),hMode==}\StringTok{"3"}\NormalTok{, }\KeywordTok{is.na}\NormalTok{(MC_n)==T)}
    \NormalTok{dat_2_5_12 <-}\StringTok{ }\NormalTok{sim_msm%>%}\KeywordTok{filter}\NormalTok{(recMode==}\KeywordTok{as.character}\NormalTok{(rset),hMode==}\StringTok{"12"}\NormalTok{, }\KeywordTok{is.na}\NormalTok{(MC_n)==T)}
    \NormalTok{dat_2_5_13 <-}\StringTok{ }\NormalTok{sim_msm%>%}\KeywordTok{filter}\NormalTok{(recMode==}\KeywordTok{as.character}\NormalTok{(rset),hMode==}\StringTok{"13"}\NormalTok{, }\KeywordTok{is.na}\NormalTok{(MC_n)==T)}
    
    \NormalTok{dat2       <-}\StringTok{ }\KeywordTok{as_tibble}\NormalTok{(dat_2_5_3)%>%}\KeywordTok{filter}\NormalTok{(age==}\DecValTok{1}\NormalTok{,!hModev2%in%}\KeywordTok{c}\NormalTok{(}\StringTok{"H12_259_cf"}\NormalTok{,}\StringTok{"H12_219_mnFcf"}\NormalTok{,}\StringTok{"H12_259_CENaivecf"}\NormalTok{))}
...
\StringTok{      }\KeywordTok{geom_line}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{future_year,}\DataTypeTok{y=}\NormalTok{F,}\DataTypeTok{color=}\NormalTok{Scenario)) +}
